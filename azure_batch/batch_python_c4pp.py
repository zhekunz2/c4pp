...
_BATCH_ACCOUNT_NAME ='***'
_BATCH_ACCOUNT_KEY = '***=='
_BATCH_ACCOUNT_URL = 'https://***.batch.azure.com'
...
def create_pool(batch_service_client, pool_id, users):
    ...
    new_pool = batch.models.PoolAddParameter(
        ...
        start_task=batchmodels.StartTask(
            ...
            command_line = "/bin/bash -c \"apt-get update; \
                apt-get install git; \
                git clone https://github.com/uiuc-arc/probfuzz.git; \
...
def add_tasks(batch_service_client, job_id, input_files, output_container_sas_url):
    ...
    print('Adding {} tasks to job [{}]...'.format(_LOW_PRIORITY_POOL_NODE_COUNT, job_id))

    tasks = list()

    #for idx, input_file in enumerate(input_files): 
    for idx in range(_LOW_PRIORITY_POOL_NODE_COUNT):
        # output_file_path='version{}.txt'.format(idx)
        # command = "/bin/bash -c \"apt-get install git; \
        #         git clone https://github.com/uiuc-arc/probfuzz.git; \
        #         sudo apt-get install -y default-jdk;\
        #         cd ./probfuzz; \
        #         ./install.sh; \
        # command = "/bin/bash -c \"git clone https://github.com/uiuc-arc/probfuzz.git; \
        #             cd ./probfuzz;\
        #             (cd language/antlr/ && wget http://www.antlr.org/download/antlr-4.7.1-complete.jar && ./run.sh); \
        #             ./check.py; \
        #             ./probfuzz.py 5\""
        command = "/bin/bash -c \"cp -r /mnt/batch/tasks/startup/wd/probfuzz ./; \
        cd probfuzz; \
        ./probfuzz.py 5\""
