...
cd $curr_model_name
pwd
if [ ! -f ${stanfile} ]; then echo "$curr_model_name fail to build!"; cp build_error.txt $org_path/${curr_model_name}_build_error.txt; exit 0; fi
...
# echo "file doesn't exist: $stan_install_dir/$curr_model_name/output_$ref.csv"
    cp stanout_${ref} $org_path/${curr_model_name}_stanout_${ref}.txt
    exit 0
fi
...
$runtime_file_path $stan_install_dir/$curr_model_name/output_${min}_1.gz | sed "s/^/$curr_model_name,/" > ${curr_model_name}/rt_0401
mv rw_* $curr_model_name
...
mv ${curr_model_name}/rt_0401 $org_path/${curr_model_name}_rt_0401.txt
