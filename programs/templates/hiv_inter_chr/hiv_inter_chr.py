...
def model(J,N,person,treatment,time,y):
    ...
    with pyro.iarange('eta1_range_'.format('')):
        eta1 = pyro.sample('eta1'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(J)]),torch.tensor(1.0)*torch.ones([amb(J)])))
    ...
    with pyro.iarange('eta2_range_'.format('')):
        eta2 = pyro.sample('eta2'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(J)]),torch.tensor(1.0)*torch.ones([amb(J)])))
    beta = pyro.sample('beta'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(1)]),torch.tensor(1.0)*torch.ones([amb(1)])))
    a1 = torch.zeros([amb(J)])
    a2 = torch.zeros([amb(J)])
...
def guide(J,N,person,treatment,time,y):
    ...
    sigma_a2 = pyro.sample('sigma_a2'.format(''), dist.StudentT(df=arg_3,loc=arg_4,scale=arg_5))
    arg_6 = pyro.param('arg_6', torch.ones((amb(1))))
    arg_7 = pyro.param('arg_7', torch.ones((amb(1))), constraint=constraints.positive)
    sigma_a1 = pyro.sample('sigma_a1'.format(''), dist.Normal(arg_6,arg_7))
    arg_8 = pyro.param('arg_8', torch.ones((amb(1))))
    arg_9 = pyro.param('arg_9', torch.ones((amb(1))), constraint=constraints.positive)
    mu_a1 = pyro.sample('mu_a1'.format(''), dist.Normal(arg_8,arg_9))
    arg_10 = pyro.param('arg_10', torch.ones((amb(J))))
    arg_11 = pyro.param('arg_11', torch.ones((amb(J))), constraint=constraints.positive)
    with pyro.iarange('eta1_prange'):
        eta1 = pyro.sample('eta1'.format(''), dist.Normal(arg_10,arg_11))
    ...
    mu_a2 = pyro.sample('mu_a2'.format(''), dist.StudentT(df=arg_12,loc=arg_13,scale=arg_14))
    arg_15 = pyro.param('arg_15', torch.ones((amb(J))), constraint=constraints.positive)
    arg_16 = pyro.param('arg_16', torch.ones((amb(J))))
    arg_17 = pyro.param('arg_17', torch.ones((amb(J))), constraint=constraints.positive)
    with pyro.iarange('eta2_prange'):
        eta2 = pyro.sample('eta2'.format(''), dist.StudentT(df=arg_15,loc=arg_16,scale=arg_17))
    arg_18 = pyro.param('arg_18', torch.ones((amb(1))))
    arg_19 = pyro.param('arg_19', torch.ones((amb(1))), constraint=constraints.positive)
    beta = pyro.sample('beta'.format(''), dist.LogNormal(arg_18,arg_19))
    for i in range(1, N+1):
        pass
    
    pass
    return { "sigma_y": sigma_y,"eta1": eta1,"eta2": eta2,"sigma_a2": sigma_a2,"sigma_a1": sigma_a1,"beta": beta,"mu_a2": mu_a2,"mu_a1": mu_a1, }
optim = Adam({'lr': 0.05})
...
print('eta1_mean', np.array2string(dist.Normal(pyro.param('arg_10'), pyro.param('arg_11')).mean.detach().numpy(), separator=','))
print('eta2_mean', np.array2string(dist.StudentT(pyro.param('arg_15')).mean.detach().numpy(), separator=','))
print('sigma_a2_mean', np.array2string(dist.StudentT(pyro.param('arg_3')).mean.detach().numpy(), separator=','))
print('sigma_a1_mean', np.array2string(dist.Normal(pyro.param('arg_6'), pyro.param('arg_7')).mean.detach().numpy(), separator=','))
print('beta_mean', np.array2string(dist.LogNormal(pyro.param('arg_18'), pyro.param('arg_19')).mean.detach().numpy(), separator=','))
print('mu_a2_mean', np.array2string(dist.StudentT(pyro.param('arg_12')).mean.detach().numpy(), separator=','))
print('mu_a1_mean', np.array2string(dist.Normal(pyro.param('arg_8'), pyro.param('arg_9')).mean.detach().numpy(), separator=','))
...
with open('samples','w') as samplefile:
    ...
    samplefile.write(np.array2string(np.array([guide(J,N,person,treatment,time,y)['eta1'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('eta2:')
    samplefile.write(np.array2string(np.array([guide(J,N,person,treatment,time,y)['eta2'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
