...
def model(x,r,n,N):
    ...
    mean_x=torch.mean(x)
    centered_x=x-mean_x
    alpha_star = pyro.sample('alpha_star'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(1)]),torch.tensor(1.0)*torch.ones([amb(1)])))
    beta = pyro.sample('beta'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(1)]),torch.tensor(10000.0)*torch.ones([amb(1)])))
    p = torch.zeros([amb(N)])
    for i in range(1, N+1):
        p[i-1]=dist.Normal(0,1).cdf(alpha_star+beta*centered_x[i-1])
...
def guide(x,r,n,N):
    ...
    alpha_star = pyro.sample('alpha_star'.format(''), dist.Cauchy(arg_1,arg_2))
...
print('alpha_star_mean', np.array2string(dist.Cauchy(pyro.param('arg_1'), pyro.param('arg_2')).mean.detach().numpy(), separator=','))
