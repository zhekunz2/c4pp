...
def model(dummy):
    y = pyro.sample('y'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(1)]),torch.tensor(3.0)*torch.ones([amb(1)])))
    with pyro.iarange('x_range_'.format('')):
        x = pyro.sample('x'.format(''), dist.Normal(torch.tensor(0.0)*torch.ones([amb(9)]),torch.exp(y/2)*torch.ones([amb(9)])))
...
def guide(dummy):
    ...
    with pyro.iarange('x_prange'):
        x = pyro.sample('x'.format(''), dist.Gamma(arg_3,arg_4))
...
print('x_mean', np.array2string(dist.Gamma(pyro.param('arg_3'), pyro.param('arg_4')).mean.detach().numpy(), separator=','))
