...
def guide(y,J,sigma):
    ...
    tau = pyro.sample('tau'.format(''), dist.Exponential(arg_3))
    arg_4 = pyro.param('arg_4', torch.ones((amb(J))), constraint=constraints.positive)
...
print('tau_mean', np.array2string(dist.Exponential(pyro.param('arg_3')).mean.detach().numpy(), separator=','))
np.set_printoptions(threshold=np.inf)
with open('samples','w') as samplefile:
    samplefile.write('mu:')
    samplefile.write(np.array2string(np.array([guide(y,J,sigma)['mu'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('tau:')
    samplefile.write(np.array2string(np.array([guide(y,J,sigma)['tau'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('theta:')
    samplefile.write(np.array2string(np.array([guide(y,J,sigma)['theta'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
