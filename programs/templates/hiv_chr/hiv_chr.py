...
def guide(y,person,J,time,N):
    ...
    sigma_a1 = pyro.sample('sigma_a1'.format(''), dist.Cauchy(arg_5,arg_6))
    ...
    mu_a2 = pyro.sample('mu_a2'.format(''), dist.Pareto(arg_9,arg_10))
    arg_11 = pyro.param('arg_11', torch.ones((amb(J))), constraint=constraints.positive)
    with pyro.iarange('eta1_prange'):
        eta1 = pyro.sample('eta1'.format(''), dist.Exponential(arg_11))
    ...
    with pyro.iarange('eta2_prange'):
        eta2 = pyro.sample('eta2'.format(''), dist.Weibull(arg_12,arg_13))
    for i in range(1, N+1):
        pass
    
    pass
    return { "sigma_y": sigma_y,"eta1": eta1,"eta2": eta2,"sigma_a2": sigma_a2,"sigma_a1": sigma_a1,"mu_a2": mu_a2,"mu_a1": mu_a1, }
optim = Adam({'lr': 0.05})
...
print('eta1_mean', np.array2string(dist.Exponential(pyro.param('arg_11')).mean.detach().numpy(), separator=','))
print('eta2_mean', np.array2string(dist.Weibull(pyro.param('arg_12'), pyro.param('arg_13')).mean.detach().numpy(), separator=','))
print('sigma_a2_mean', np.array2string(dist.Normal(pyro.param('arg_3'), pyro.param('arg_4')).mean.detach().numpy(), separator=','))
print('sigma_a1_mean', np.array2string(dist.Cauchy(pyro.param('arg_5'), pyro.param('arg_6')).mean.detach().numpy(), separator=','))
print('mu_a2_mean', np.array2string(dist.Pareto(pyro.param('arg_9'), pyro.param('arg_10')).mean.detach().numpy(), separator=','))
...
with open('samples','w') as samplefile:
    ...
    samplefile.write(np.array2string(np.array([guide(y,person,J,time,N)['eta1'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('eta2:')
    samplefile.write(np.array2string(np.array([guide(y,person,J,time,N)['eta2'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
