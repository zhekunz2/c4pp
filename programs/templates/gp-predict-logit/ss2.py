...
def guide(T):
    ...
    for t in range(2, T+1):
        arg_9 = pyro.param('arg_9', torch.ones((amb(T))), constraint=constraints.positive)
        arg_10 = pyro.param('arg_10', torch.ones((amb(T))), constraint=constraints.positive)
        with pyro.iarange('h_prange'):
...
with open('pyro_out', 'w') as outputfile:
    ...
    outputfile.write('h, gamma,{0},{1}\n'.format(pyro.param('arg_9').detach().numpy(),pyro.param('arg_10').detach().numpy()))
    outputfile.write('phi, uniform,{0},{1}\n'.format(pyro.param('arg_1').detach().numpy(),pyro.param('arg_2').detach().numpy()))
