...
def guide(history):
    ...
    with pyro.iarange('p_prange'):
        p = pyro.sample('p'.format(''), dist.Gamma(arg_1,arg_2))
    arg_3 = pyro.param('arg_3', torch.ones((amb(2))), constraint=constraints.positive)
    arg_4 = pyro.param('arg_4', torch.ones((amb(2))), constraint=constraints.positive)
    with pyro.iarange('phi_prange'):
        phi = pyro.sample('phi'.format(''), dist.Gamma(arg_3,arg_4))
...
print('p_mean', np.array2string(dist.Gamma(pyro.param('arg_1'), pyro.param('arg_2')).mean.detach().numpy(), separator=','))
print('phi_mean', np.array2string(dist.Gamma(pyro.param('arg_3'), pyro.param('arg_4')).mean.detach().numpy(), separator=','))
np.set_printoptions(threshold=np.inf)
with open('samples','w') as samplefile:
    samplefile.write('p:')
    samplefile.write(np.array2string(np.array([guide(history)['p'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('phi:')
    samplefile.write(np.array2string(np.array([guide(history)['phi'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
