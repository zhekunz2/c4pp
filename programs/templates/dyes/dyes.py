...
def model(y,BATCHES,SAMPLES):
    ...
    sigmasq_between = torch.zeros([amb(1)])
    sigmasq_within = torch.zeros([amb(1)])
...
def guide(y,BATCHES,SAMPLES):
    ...
    theta = pyro.sample('theta'.format(''), dist.Gamma(arg_1,arg_2))
    ...
    tau_between = pyro.sample('tau_between'.format(''), dist.Pareto(arg_3,arg_4))
    ...
    pass
    return { "mu": mu,"theta": theta,"tau_within": tau_within,"tau_between": tau_between, }
optim = Adam({'lr': 0.05})
...
print('theta_mean', np.array2string(dist.Gamma(pyro.param('arg_1'), pyro.param('arg_2')).mean.detach().numpy(), separator=','))
print('tau_within_mean', np.array2string(dist.Gamma(pyro.param('arg_5'), pyro.param('arg_6')).mean.detach().numpy(), separator=','))
print('tau_between_mean', np.array2string(dist.Pareto(pyro.param('arg_3'), pyro.param('arg_4')).mean.detach().numpy(), separator=','))
np.set_printoptions(threshold=np.inf)
with open('samples','w') as samplefile:
    samplefile.write('mu:')
    samplefile.write(np.array2string(np.array([guide(y,BATCHES,SAMPLES)['mu'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('theta:')
    samplefile.write(np.array2string(np.array([guide(y,BATCHES,SAMPLES)['theta'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('tau_within:')
    samplefile.write(np.array2string(np.array([guide(y,BATCHES,SAMPLES)['tau_within'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('tau_between:')
    samplefile.write(np.array2string(np.array([guide(y,BATCHES,SAMPLES)['tau_between'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
