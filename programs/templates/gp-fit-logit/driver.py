...
def callstansummary(filename):
    import subprocess
    print(filename)
    process = subprocess.Popen('/home/saikat/projects/TestMin/cmdstan-2.14.0/bin/stansummary {0}'.format(filename), stdout=subprocess.PIPE, shell=True)
...
if inf_type == 'sampling':
    ...
    with open(paramfile.format("nuts", iterations), 'w') as outputfile:
    ...
    chains = fit.sim['samples'][0].chains
    np.set_printoptions(threshold=np.inf, linewidth=np.inf)
    with open(paramfile.format("hmc", iterations), 'w') as outputfile:
        for i in range(0, len(fit.sim['fnames_oi'])):
            param = fit.sim['fnames_oi'][i]
            if param == 'lp__':
                continue
    ...
    print(result[0])
    np.set_printoptions(threshold=np.inf, linewidth=np.inf)
    with open(paramfile.format("vb", iterations), 'w') as outputfile:
