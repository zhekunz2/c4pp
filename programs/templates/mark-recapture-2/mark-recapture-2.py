...
M=torch.tensor(M)
def model(C,R,M):
    theta_max = torch.zeros([amb(1)])
    theta_max=M
    theta_max=theta_max/(C-R+M)
    theta = pyro.sample('theta'.format(''), dist.Normal(torch.tensor(1234.0)*torch.ones([amb(1)]),torch.tensor(1234.0)*torch.ones([amb(1)])))
    pyro.sample('obs__100'.format(), dist.Binomial(C,theta), obs=R)
    N = torch.zeros([amb(1)])
    N=M/theta
...
with open('samples','w') as samplefile:
    ...
    samplefile.write(np.array2string(np.array([guide(C,R,M)['theta'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
