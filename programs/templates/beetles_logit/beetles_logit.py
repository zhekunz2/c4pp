...
def model(x,r,n,N):
    ...
    pyro.sample('obs__100'.format(), dist.Binomial(total_count=n,logits=m), obs=r)
    alpha = torch.zeros([amb(1)])
    p = torch.zeros([amb(N)])
    llike = torch.zeros([amb(N)])
    rhat = torch.zeros([amb(N)])
    for i in range(1, N+1):
        p[i-1]=(1/(1+torch.exp(-m[i-1])))
...
def guide(x,r,n,N):
    ...
    alpha_star = pyro.sample('alpha_star'.format(''), dist.Weibull(arg_1,arg_2))
    arg_3 = pyro.param('arg_3', torch.ones((amb(1))))
    arg_4 = pyro.param('arg_4', torch.ones((amb(1))), constraint=constraints.positive)
    beta = pyro.sample('beta'.format(''), dist.Cauchy(arg_3,arg_4))
    for i in range(1, N+1):
        pass
...
print('beta_mean', np.array2string(dist.Cauchy(pyro.param('arg_3'), pyro.param('arg_4')).mean.detach().numpy(), separator=','))
print('alpha_star_mean', np.array2string(dist.Weibull(pyro.param('arg_1'), pyro.param('arg_2')).mean.detach().numpy(), separator=','))
