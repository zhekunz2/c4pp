...
transformed parameters {
  ...
  pie  = mu_pr[4] + sigma[4] * pi_pr;
  rho = exp(mu_pr[5] + sigma[5] * rho_pr);
}

model {
// gng_m4: RW(rew/pun) + noise + bias + pie model (M5 in Cavanagh et al 2013 J Neuro)
  // hyper parameters
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      wv_g[cue[i, t]]  = qv_g[cue[i, t]] + b[i] + pie[i] * sv[cue[i, t]];
      ...
      {  // noise
        pGo[cue[i, t]]   =pGo[cue[i, t]]   * (1 - xi[i]);
        pGo[cue[i, t]]   =pGo[cue[i, t]]   + xi[i]/2;
      }
