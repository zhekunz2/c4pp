...
N=46
N=torch.tensor(N)
def model(diam1,diam2,canopy_height,weight,N):
    ...
    log_weight=dist.Normal(beta[0-1]+beta[2-1]*log_canopy_volume,sigma)
    
def guide(diam1,diam2,canopy_height,weight,N):
...
for i in range(4000):
    loss = svi.step(diam1,diam2,canopy_height,weight,N)
    if ((i % 1000) == 0):
        print(loss)
...
with open('samples','w') as samplefile:
    ...
    samplefile.write(np.array2string(np.array([guide(diam1,diam2,canopy_height,weight,N)['beta'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
    samplefile.write('sigma:')
    samplefile.write(np.array2string(np.array([guide(diam1,diam2,canopy_height,weight,N)['sigma'].data.numpy() for _ in range(1000)]), separator=',').replace('\n',''))
    samplefile.write('\n')
