...
class SummTrueMetric:
    ...
    def y_wass(self, metrics=[], thresholds=[], var_check=False):
        ...
        print(wasserstein_distance(joint_summ_truth.Mean,joint_summ_truth.Truth))
    def y_mse(self, metrics=[], thresholds=[], var_check=False):
        joint_summ_truth = self.summ_a.join(self.truth)
        joint_summ_truth.dropna(subset=['Mean', 'Truth'],inplace=True)
        from sklearn.metrics import mean_squared_error
        print(mean_squared_error(joint_summ_truth.Truth, joint_summ_truth.Mean))
...
class Summary:
    def __init__(self, summary_file, runtime=False, robust=False):
        ...
        if not runtime and not robust:
            ...
            self.summary_df = summary_df.drop([xx for xx in list(summary_df.index) if "__" in xx or "robust_local" in xx or "robust_hyper" in xx or "robust_weight" in xx or "aug_link" in xx or "robust_const" in xx or "robust_" in xx])
            #self.runtime_df = summary_df.drop([xx for xx in list(summary_df.index) if "__" not in xx])
...
def csv_metric(csv_a, csv_b, metrics, thresholds, opt=[], var_check=False):
    ...
    if var_check:
        var_check_ret = False
    param_set = [pp for pp in param_set if ("robust_weight" not in pp and "robust_local" not in pp and "robust_const" not in pp and "robust_" not in pp)]
    for param in param_set:
...
if __name__ == "__main__":
    ...
    pp.add_argument("-d", "--debug", action="store_true", default=False,
            help="Print the metric results for each parameter")
