...
def predict(clf, X_test, Y_test, weighted=False):
    ...
    print(">>>>", ','.join(str(int(x)) for x in predictions))# (prediction_probs[:,1] >= 0.7)))
    print("<<<<", ','.join(str(int(x)) for x in Y_test))
...
def run_rf(X_train, Y_train, X_test, Y_test, feature_names, cv=False,  trees=20, validation=False, learning=False,
    ...
    if feature_selection:
        ...
        for k in [10, 20, 40, 60, 80, 100, 250]:
            rfconfig = getRunConfig(args.feature_file, args.metric)
        ...
        clf = RandomForestClassifier(n_estimators=50, min_samples_split=6, criterion='gini', max_features=10,
                                     max_depth=None, class_weight='balanced' if balance_by_weight else None, n_jobs=-1)
...
ignore_indices=['vi_.*'] if args.ignore_vi else []
...
thresholds={ 'kl': [0.1, 1, 2, 3, 4, 5],
             ...
             'rhat_min' : [1.1, 1.2,  1.4, 1.6,  1.8,  2.0],
             'ess_min' : list(np.arange(0.01, 0.09, 0.01)),
             'wass' : list(np.arange(0.1, 1.0, 0.1)),
...
if args.test_features is not None and args.test_labels is not None:
...
if type(threshold) == list:
    ...
    if split_by_class is not None:
        ...
        if args.special_index is not None:
            ...
            Y_test.append(full_table.ix[args.special_index][metric_label])
